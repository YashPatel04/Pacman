...
void GhostManager::reset(unsigned char i_level, const std::array<Position, 4> &i_ghost_positions) {
    ...
    //use the blue host to get the location of the house and the red ghost to get the location of the exit
    for(Ghost& ghost : ghosts){
...
void GhostManager::update(unsigned char i_level, std::array<std::array<Cell, MAP_HEIGHT>, MAP_WIDTH> &i_map,
                          Pacman &i_pacman) {
    if(0 == i_pacman.get_energizer_timer()){ //Don't uppate the timer when Pacman is energized
        if(wave_timer == 0){
            if(current_wave<7){
                current_wave++;
                for(Ghost& ghost : ghosts){
